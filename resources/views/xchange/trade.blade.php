...
<input type="text" id="from_coin_decimals" value="{{$from_coin_decimals}}" >
<input type="text" id="to_coin_decimals" value="{{$to_coin_decimals}}" >
				...
				<div class="col-sm-12 col-md-4 m-b-10 p-l-10">
					<div class="btc-tabs margin-top-10">
						...
						<div class="tab-content">
									...
									<tbody class="btc-pair" align="left">
										<?php
											foreach($pair_list['btc'] as $coin)
											{
												...
												?>
												<tr>
													<td align="left"><p class="text-muted no-margin"><a href="{{url('/trade/btc-'.$coin->pair_coin)}}">
													<?php echo $fontclorS; ?>
													{{strtoupper($coin->pair_coin)}}/BTC
													<?php echo $fontclorE;  ?>
													</a></p></td>
													<td align="left"><p class="no-margin"><span></span> {{number_format($coin->rate,4)}}</p></td>
									...
									<tbody class="eth-pair" align="left">
										<?php
											foreach($pair_list['eth'] as $coin)
											{
												...
												?>
												<tr>
													<td align="left"><p class="text-muted no-margin"><a href="{{url('/trade/eth-'.$coin->pair_coin)}}">
													<?php echo $fontclorS; ?>
													{{strtoupper($coin->pair_coin)}}/ETH
													<?php echo $fontclorE;  ?>
													</a></p></td>
													<td align="left"><p class="no-margin"><span></span> {{number_format($coin->rate,4)}}
									...
									<tbody class="inr-pair" align="left">
										<?php
											foreach($pair_list['inr'] as $coin)
											{
												...
												?>
												<tr>
													<td align="left"><p class="text-muted no-margin"><a href="{{url('/trade/inr-'.$coin->pair_coin)}}">
													<?php echo $fontclorS; ?>{{strtoupper($coin->pair_coin)}}/INR 
							...
							<div role="tabpanel" class="tab-pane {{strtolower($from_currency)=='gix'?'active':''}}" id="gix">
								<div class="dash-table-markets">
								<table id="data-tables-market" class="table table-cryptic <!--dataTable--> no-footer"> <!--data-page-length="10"-->
									<thead align="left">
										 <tr>
									...
									<tbody class="gix-pair"  align="left">
										<?php
											foreach($pair_list['gix'] as $coin)
											{
												...
												?>
												<tr>
													<td align="left"><p class="text-muted no-margin"><a href="{{url('/trade/gix-'.$coin->pair_coin)}}">
													<?php echo $fontclorS; ?>
													{{strtoupper($coin->pair_coin)}}/GIX
													<?php echo $fontclorE;  ?>
													</a></p></td>
													<td align="left"><p class="no-margin"><span></span> {{number_format($coin->rate,4)}}</p></td>
													<!-- <td class="no-wrap"><span class="change-down color-red label label-light-danger"><i class="fa fa-chevron-down"></i> -15.20%</span></td> -->
										...
										<h3>Buy {{strtoupper($to_currency)}}</h3>
											<form action="#" name="buy-form" id="buy-form" autocomplete="off">
												...
												<div class="discover-btn no-shadow-s clearfix">
													<button type="submit" class="btn btn-global-dark buy buy-btn">Buy {{strtoupper($to_currency)}} <span><img src="{{asset('/public/gilt/img/btn-arrow.png')}}"/></span></button>
												</div>
...
@section('footerjs')
    ...
    <script src="{{$node_server_point}}/socket.io/socket.io.js"></script>
