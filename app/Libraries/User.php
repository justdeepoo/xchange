...
class User
{
    ...
    public static function userData($data){
        ...
        $d = UserModel::where(['id'=>$data['id']])->select(['first_name', 'last_name', 'email', 'mobile', 'dob', 'mobile_verified', 'kyc', 'is_profile', 'is_kyc', 'is_address', 'is_bank', 'auth_code_url', 'auth_enabled', 'document_attached', 'document_number', 'document_issue_date', 'document_exp_date', 'country', 'is_iddoc_issue_date', 'is_iddoc_exp_date'])->first();
        $u = [
            'user'=>$d
        ];
    ...
    public static function userBank($user_id){
        return $d = DB::table('user_bank_accounts')->where(['user_id'=>$user_id])->select(['ifsc', 'bank', 'linked_mobile', 'account_no', 'holder_name', 'pan_card_number', 'branch', 'account_type', 'is_submit', 'color_scan_cheque'])->first();
    }
       ...
       if(isset($d['user_id']) && isset($d['locked_bal']) && isset($d['coin']))
        {
            ...
            $coin = strtolower(trim($d['coin']));

            if($coin!='inr')
            {
                $l_a = User::convert_unit($coin, $d['locked_bal']);
                $c_a = User::convert_unit($coin, $d['balance']);
            }
            $s = WalletModel::where(['user_id'=>$d['user_id'], 'coin'=>$coin])->update([
    ...
    {	
		if(isset($d['orderby_key']) && isset($d['orderby_value'])){			
			return $resp = DB::table($d['table'])->orderBy($d['orderby_key'], $d['orderby_value'])->where($d['where'])->select($d['select'])->get();	
		}else{
	...
	public static function saveContactus($data, $event = 'Sent Successfully')
    {
         return $resp = DB::table($data['table'])->insert($data['data']);
    }	
	
	public static function saveSubscriberNews($data, $event = 'Sent Successfully')
    {
         return $resp = DB::table($data['table'])->insert($data['data']);
    }	
	
	public static function saveAddtokenSubs($data, $event = 'Sent Successfully')
    {
         return $resp = DB::table($data['table'])->insert($data['data']);
    }

	public static function saveCustomersupport($data, $event = 'Sent Successfully')
    {
         return $resp = DB::table($data['table'])->insert($data['data']);
    }	
	...
	public static function getHeaderNotification($user_id=""){
		$d = ['table'=>'notifications', 'where'=>['user_id'=>$user_id], 'select'=>['subject','message'], 'orderby_key' => 'id', 'orderby_value' => 'desc' ];
		return User::getData($d);
	}
	public static function getTradeHistory($user_id="") {
	...
	public static function getDepositRequests($user_id="")
	{
		$query = "select * from deposit_request where 1 and status in (0,1,2) ";
		if(!empty($user_id)){
			$query .= " and user_id = '".$user_id."' ";
		}
		$query .= " order by id desc ";
		return $resp = DB::select($query);
	}
	public static function getWithdrawRequests($user_id="")
	{
		$query = "select * from withdraw_requests where 1 and status in (0,1,2) ";
		if(!empty($user_id)){
			$query .= " and user_id = '".$user_id."' ";
		}
		$query .= " order by id desc ";
	...
	public static function updateDepositRequest($data)
	{
		$resp = DB::table($data['table'])->where($data['where'])->update($data['data']);	
	}
	public static function updateWithdrawRequest($data)
	{
		$resp = DB::table($data['table'])->where($data['where'])->update($data['data']);	
	}
